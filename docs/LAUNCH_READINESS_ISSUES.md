...
- [ ] Mayday webhook needs end-to-end verification: dependency, settings endpoint, trigger, retry/failure logging, and user-facing status. Unit coverage now verifies settings URL validation, trigger payload posting, and unsafe stored URL skipping; a real outbound Discord/Slack smoke and user-facing delivery status still need verification.
- [ ] New player onboarding needs a verified happy path: guest/tutorial/login, mine, smelt, craft/equip, sell/claim, and repair.
...
- [ ] Large frontend files should be split after blockers are fixed: `frontend/ui.js`, `frontend/index.html`, `frontend/renderer.js`, and `frontend/terminal.js`.
...
- [ ] Add browser smoke coverage for login/guest, world view, terminal commands, market, missions, contracts, corp, arena, and wiki. Latest attempt was blocked by the local automation runtime failing before command execution with `failed to write kernel assets`; prior browser smoke verified guest login and core panels before the most recent frontend hardening pass.
- [ ] Align `/api/commands`, terminal help, GDD, and the actual intent/API handlers. Stale `FIELD_TRADE` and `ARENA_REGISTER` terminal commands were removed, refueler bots now queue `TRANSFER`, and `/api/commands` distinguishes immediate endpoint commands. GDD and broader docs still need a full alignment pass.
...
- Python source syntax compile passed for 161 files using in-memory `compile(...)` because local `__pycache__` writes were locked.
- `python -m pytest tests -q` passed with 41 tests and the known SQLAlchemy deprecation warning plus a `.pytest_cache` warning. The command used `C:\Users\cosmi\AppData\Local\Python\bin\python.exe` because the Windows `python` app alias was inaccessible.
- `node --check` passed for main frontend JavaScript modules, and the inline script in `frontend/admin.html` parsed successfully with Node.
...
- Latest browser automation attempt is still blocked before any browser command executes with `failed to write kernel assets: The system cannot find the path specified. (os error 3)`.
