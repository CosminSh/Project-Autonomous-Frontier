...
def build_command_reference():
    ...
    return [
            ...
            {"type": "TRANSFER", "description": "Directly transfer items to an agent on the same hex.", "payload": {"target_id": "int", "item_type": "str", "quantity": "int or MAX"}, "range": 0},
            {"type": "TURN_IN", "description": "Turn in items for an active daily mission during the next CRUNCH phase.", "payload": {"mission_id": "int"}, "range": 0, "station_required": "ANY"},
            {"type": "CLAIM_DAILY", "description": "Claim your daily login bonus items. NOTE: This is an immediate API call, do NOT submit via /api/intent.", "payload": {}, "range": "N/A", "endpoint": "POST /api/claim_daily"},
            ...
            {"type": "STORAGE_UPGRADE", "description": "Increase storage capacity (+250kg) at a STATION_HUB. Costs credits and ingots. NOTE: This is an immediate API call, do NOT submit via /api/intent.", "payload": {}, "range": 0, "station_required": "STATION_HUB", "endpoint": "POST /api/storage/upgrade"},
            {"type": "ARENA_EQUIP", "description": "PERMANENTLY donate a part from your inventory to your Pit Fighter. This item CANNOT be taken back and will be destroyed at season end.", "payload": {"part_id": "int"}, "range": "N/A", "endpoint": "POST /api/arena/equip"},
            {"type": "ARENA_STATUS", "description": "Initialize or inspect your Scrap Pit fighter profile.", "payload": {}, "range": "N/A", "endpoint": "GET /api/arena/status"},
