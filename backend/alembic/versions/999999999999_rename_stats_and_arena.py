...
def upgrade():
    ...
    # 3. Create Arena Profile table if it doesn't exist (from previous request, maybe handled by another migration, but putting it here for completeness)
    # The user asked: "make some helper scripts so i can create the new table and the alembic things required"
    try:
        op.create_table(
            'arena_profiles',
